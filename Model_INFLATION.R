...
#Define file path
library(readr)
act_data <- read_delim("act_data_INF.csv", 
...
data2 <- act_data[1:52,]
data2_INFL = data2[,1]
data2_INFL <- as.data.frame(data2_INFL)
data2_INFL <- data2_INFL %>% 
  rename(PCECTPI = data2_INFL)

data2_4 <- act_data[1:55,]
data4_INFL = data2_4[,1]
data4_INFL <- as.data.frame(data4_INFL)
data4_INFL <- data4_INFL %>% 
  rename(PCECTPI = data4_INFL)


##########AR(1) MODEL: BASELINE##############
INFL <- act_data$PCECTPI #extracts time series "PCECTPI" from the data frame and assigns it to the variable INFL
INFL <- as.data.frame(INFL) #convert INFL object into a data frame

train_INFL <- window(INFL$INFL, end = 52) #training set 2/3 of the data
test_INFL <- window(INFL$INFL, start = 53) #testing set 1/3 of the data

fit <- arima(train_INFL, order = c(1,0,0)) #fits an ARIMA (AutoRegressive Integrated Moving Average) model to the train_INFL dataset. 
...
w_size <- length(train_INFL) #calculates the length (number of obs) of train_INFL and assigns it to the variable w_size.

#1-step ahead forecast
h <- 1 ## define the forecast horizon (in this case, 1 step ahead)
n <- length(test_INFL)-h+1 #number of forecasts to make
...
for (i in 1:n){     #loop from 1 to n 
  x <- INFL[i:(w_size-1+i),1]      #In each iteration, it extracts a portion of the INFL time series, which is x. This portion is defined by the index range from i to w_size - 1 + i. 
...
mean((fcmat1 - INFL[53:78,1])^2) ##calculate the mean squared error (MSE) of the forecast
#calculates the mean squared error (MSE) by comparing the 1-step ahead forecasts stored in fcmat1 with the actual values from INFL for the corresponding time periods (i.e., INFL[53:78, 1]), and then takes the mean of the squared differences. 
#0.4154806

#4-step ahead forecast
h <- 4
n <- length(test_INFL)-h+1
fcmat4 <- matrix(0, nrow = n, ncol = h)

for (i in 1:n){
  x <- INFL[i:(w_size-1+i),1]
  refit <- Arima(x, order = c(1,0,0))
  fcmat4[i,]<- forecast(refit, h = h)$mean
}

mean((fcmat4[,4] - INFL[56:78,1])^2) #0.4434229
...
#1-step ahead forecast 
FHLR1 <- fcast_chi1 %>%   
  select(PCECTPI)         #select the 1-step ahead forecast for the INFL variable 

train_INFL <- as.data.frame(train_INFL)
train_INFL <- train_INFL %>% #converts the train_INFL object into a data frame
  rename(PCECTPI = train_INFL)      #rename the column in the train_INFL data frame to "PCECTPI." 

# Graph
FHLR1 <- rbind(train_INFL, FHLR1)

plot.ts(FHLR1, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Quarters")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=3, h=0)     #vertical line at 52
title("FHLR1 (1-step ahead)")

#MSE
mean((FHLR1[53:78,] - act_data[53:78,]$PCECTPI)^2)
#0.5141131


#4-step ahead forecast 
FHLR1.4step <- fcast_chi4 %>% 
  select(PCECTPI)               #extracts the 4-step ahead forecasts for the "PCECTPI" variable from the data frame fcast_chi4
...
FHLR1.4step <- FHLR1.4step %>% 
  rename(PCECTPI = FHLR1.4step)       #renames the column in the FHLR1.4step data frame to "PCECTPI."

#Graph
FHLR1.4step <- rbind(data4_INFL, FHLR1.4step)    #combining them into a single data frame: to plot historical data alongside forecasts for comparison

plot.ts(FHLR1.4step, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Quarters")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=2, h=0)
title("FHLR1 (4-step ahead)")

#MSE
mean((FHLR1.4step[56:78,] - act_data[56:78,]$PCECTPI)^2)
#0.4773381
...
xi_INFL <- xi_rol$PCECTPI
xi_INFL <- as.data.frame(xi_INFL)
xi_INFL_train <- window(xi_INFL$xi_INFL, end = 52) #rolling window of size 52 for the "xi_INFL" variable

fit <- arima(xi_INFL_train, order = c(1,0,0)) #fit AR(1) model to rolling window of xi
w_size <- length(xi_INFL_train) #length of the training data
...
xi_INFL.iter <- matrix(0, nrow = n, ncol = h)

# Loop over the number of forecasts.
for (i in 1:n){ 
  x <- xi_INFL[1+(w_size * (i-1)):(w_size * i),]      #extract the appropriate window of data for forecasting
  refit <- Arima(x, order = c(1,0,0))          #refit an AR(1) with the new data
  xi_INFL.iter[i,] <- xi_INFL[(w_size  * (i+h)), 1]     #store the actual value for the next quarter
...
mean((fcmat1.xi - xi_INFL.iter)^2, na.rm = TRUE) #0.2066698

# Combine forecasts (x = chi + xi).
INFL_fhlr2 = FHLR1[53:78,] + fcmat1.xi
INFL_fhlr2 <- as.data.frame(INFL_fhlr2)
INFL_fhlr2 <- INFL_fhlr2 %>% 
  rename( PCECTPI = V1)

# Create a new time series, "FHLR2," by combining "train_INFL" and "INFL_fhlr2."
FHLR2 <- rbind(train_INFL, INFL_fhlr2) 

#Plot
plot.ts(FHLR2, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Time (Quarters)")
points(act_data$PCECTPI, type = "l", col = 1)   #overlay actual values on the plot
abline(v="52", col=7, lty=3,h=0)
title("FHLR2 (1-step ahead)")

#MSE of the combined forecasts.
mean((FHLR2[53:78,] - act_data[53:78,]$PCECTPI)^2)  
#0.5276394
...
xi_INFL4.iter <- matrix(0, nrow = n, ncol = 1)


for (i in 1:n){
  x <- xi_INFL[1+(w_size * (i-1)):(w_size * i),]
  refit <- Arima(x, order = c(1,0,0))
  xi_INFL4.iter[i,] <- xi_INFL[(w_size  * (i+h)), 1]
  fcmat4.xi[i,]<- forecast(refit, h = h)$mean
}

mean((fcmat4.xi[,4] - xi_INFL4.iter)^2, na.rm = TRUE) #0.19622544 MSE for xi's

#x=chi+xi
INFL_fhlr2.4step = FHLR1.4step[56:78,] + fcmat4.xi[,4]
INFL_fhlr2.4step <- as.data.frame(INFL_fhlr2.4step)
INFL_fhlr2.4step <- INFL_fhlr2.4step %>% 
  rename( PCECTPI = INFL_fhlr2.4step)

#Graph
FHLR2.4step <- rbind(data4_INFL, INFL_fhlr2.4step) 
plot.ts(FHLR2.4step, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Time (Quarters)")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=3,h=0)
title("FHLR2 (4-step ahead)")

#MSE
mean((INFL_fhlr2.4step[,1] - act_data[56:78,1])^2) 
#0.4799853
...
mean((ML1_1[,1] - act_data[53:78,1])^2) #0.511829

#Graph
ML1_1g = rbind(data2_INFL, ML1_1)

plot.ts(ML1_1g, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Time (Quarters)")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=3,h=0)
...
#MSE
mean((ML1_4 - act_data[56:78,1])^2) #0.4773381

#Graph
ML1_4 <- as.data.frame(ML1_4)
ML1_4 <- ML1_4 %>% 
  rename(PCECTPI = ML1_4)

ML1_4g <- rbind(data4_INFL, ML1_4)

plot.ts(ML1_4g, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Time (Quarters)")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=3,h=0)
...
for (i in 1:n){
  ML2 <- gbm(PCECTPI ~ ., data = xi_rol[1+(w_size * (i-1)):(w_size * i),],
...
#MSE
mean((ML2_1[,1] - act_data[53:78,1])^2) #0.5233025

#Graph
ML2_g = rbind(data2_INFL, ML2_1)

plot.ts(ML2_g, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Time (Quarters)")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=3,h=0)
...
# MSE
mean((ML2_4 - act_data[56:78,1])^2) #0.494578

# Graph 
ML2_4 <- as.data.frame(ML2_4)
ML2_4 <- ML2_4 %>% 
  rename(PCECTPI = ML2_4)

ML2_g <- rbind(data4_INFL, ML2_4)
...
#MSE
mean((x_fcmat1.lasso - x_testmat.lasso[,1])^2)  #0.0008930617

#Graph
DGR1_1 = x_fcmat1.lasso[,1]
DGR1_1 <- as.data.frame(DGR1_1)
DGR1_1 <- DGR1_1 %>% 
  rename(PCECTPI = DGR1_1)

DGR1_1g = rbind(data2_INFL, DGR1_1)

plot.ts(DGR1_1g, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Time (Quarters)")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=3,h=0)
...
#MSE
mean((x_fcmat4.lasso[,4] - x_testmat.lasso[,1])^2) #0.0009776468
...
DGR1_4 <- DGR1_4 %>% 
  rename(PCECTPI = DGR1_4)

DGR1_4g <- rbind(data4_INFL, DGR1_4)

plot.ts(DGR1_4g, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Time (Quarters)")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=3,h=0)
...
for (i in 1:n){
  ...
  set.seed(12)
  DGR2 <- gbm(PCECTPI ~ ., data = act_data[i:w_size+i-1,] ,    #training data
...
#MSE
mean((fcmat1.boost[,1] - act_data[53:78,1])^2) #0.2464105
...
DGR2 <- rbind(data2_INFL, fcmat1.boost)

plot.ts(DGR2, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Time (Quarters)")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=3, h=0)
...
for (i in 1:n){
  ...
  set.seed(12)
  DGR2 <- gbm(PCECTPI ~ ., data = act_data[i:w_size+i-1,], cv.folds = 10, 
...
#MSE
mean((fcmat4.boost[,4] - act_data[56:78,1])^2) #0.3127116

#Graph
DGR2_4 = fcmat4.boost[,4]
DGR2_4 <- as.data.frame(DGR2_4)
DGR2_4 <- DGR2_4 %>% 
  rename(PCECTPI = DGR2_4)

DGR2_4 <- rbind(data4_INFL, DGR2_4)

plot.ts(DGR2_4, col=6, xlim = c(0, 78), ylim = c(-5,5), ylab="PCECTPI", xlab="Time (Quarters)")
points(act_data$PCECTPI, type = "l", col = 1)
abline(v="52", col=7, lty=3, h=0)
...
###AR1
#1-step
AR_1 <- mean((fcmat1 - INFL[53:78,1])^2) #0.4516197
#4-step
AR_4 <- mean((fcmat4[,4] - INFL[56:78,1])^2) #0.4218973

###FHLR1
#1-step
FHLR1_1 <- mean((FHLR1[,1] - act_data[53:78,]$PCECTPI)^2) #0.3868886
#4-step
FHLR1_4 <- mean((FHLR1.4step[56:78,] - act_data[56:78,]$PCECTPI)^2) #0.2801673

###FHLR2
#1-step
FHLR2_1 <- mean((FHLR2[53:78,] - act_data[53:78,]$PCECTPI)^2) #0.3756181
#4-step
FHLR2_4 <- mean((INFL_fhlr2.4step[,1] - act_data[56:78,1])^2) #0.2809238
