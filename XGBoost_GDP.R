...
for (i in 1:n_windows) {
  ...
  train_labels <- train_data[, "GDPC1"]
  train_feat <- train_data[, -which(colnames(train_data) == "GDPC1")]
  
  test_data <- as.matrix(act_data[-(window_start:(window_start + window_size - 1)), ])
  test_labels <- test_data[, "GDPC1"]
  test_feat <- test_data[, -which(colnames(test_data) == "GDPC1")]
...
matplot(time_steps, cbind(test_labels, predictions), type = "l", lty = 1,
        col = c("black", "red"), lwd = 2, xlab = "Quarters", ylab = "GDP",
        main = "Actual vs. 1-step ahead XGBoost prediction")

# Add a legend to the plot
legend("topright", legend = c("Actual", "Predicted"), col = c("black", "red"), lty = 1, lwd = 2)
