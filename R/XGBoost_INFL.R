...
for (i in 1:n_windows) {
  ...
  train_labels <- train_data[, "PCECTPI"]
  train_feat <- train_data[, -which(colnames(train_data) == "PCECTPI")]
  
  test_data <- as.matrix(act_data[-(window_start:(window_start + window_size - 1)), ])
  test_labels <- test_data[, "PCECTPI"]
  test_feat <- test_data[, -which(colnames(test_data) == "PCECTPI")]
...
matplot(time_steps, cbind(test_labels, predictions), type = "l", lty = 1,
        col = c("black", "pink"), lwd = 2, xlab = "Quarters", ylab = "INFL",
        main = "Actual vs. 1-step ahead XGBoost prediction")

# Add a legend to the plot
legend("topright", legend = c("Actual", "Predicted"), col = c("black", "pink"), lty = 1, lwd = 2)
